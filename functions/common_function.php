...
function get_unique_categories(){
  ...
  $select_query=" select * from  `products` where category_id=$category_id";
      $result_query=mysqli_query($con,$select_query);
        ...
        if($num_of_rows==0){
          echo "<h2 class='text-danger mx-auto'>No stock for this category</h2>";
        }
      while($row=mysqli_fetch_assoc($result_query)){
        $product_id=$row['product_id'];
        $product_title=$row['product_title'];
...
function get_unique_brands(){
  ...
  $select_query=" select * from  `products` where brand_id=$brand_id";
      $result_query=mysqli_query($con,$select_query);
        $num_of_rows=mysqli_num_rows($result_query);
        if($num_of_rows==0){
          echo "<h2 class='text-danger mx-auto'>This brand is not available for service</h2>";
        }
      while($row=mysqli_fetch_assoc($result_query)){
        $product_id=$row['product_id'];
        $product_title=$row['product_title'];
...
function search_product(){
  ...
  $search_query=" select * from  `products` where product_keywords like '%$search_data_value%'";
      ...
      $num_of_rows=mysqli_num_rows($result_query);
        if($num_of_rows==0){
          echo "<h2 class='text-danger mx-auto'>No results match. No products found on this category!</h2>";
        }
      while($row=mysqli_fetch_assoc($result_query)){
        $product_id=$row['product_id'];
        $product_title=$row['product_title'];
...
// this function add item to the cart and also check if item already present in the add.
// cart function
function cart(){
...
$num_of_rows=mysqli_num_rows($result_query);
        if($num_of_rows>0){
          ...
          echo "<script>alert('Item is added to add')</script>";
          echo "<script>window.open('index.php','_self')</script>";
        }
...
// this function shows the superscript n0. what is in the cartt
function cart_item(){
  if(isset($_GET['add_to_cart'])){
    global $con;
    $get_ip_add = getIPAddress();   
      ...
      if(!isset($_GET['my_orders'])){
        if(!isset($_GET['delete_account'])){
          $get_orders="select * from `user_orders`
           ...
           if($row_count>0){
            echo"<h3 class='text-center text-success mt-5 mb-2'>You have <span class='text-danger'>
